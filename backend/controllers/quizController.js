...
export const getQuizResults = async (req, res, next) => {
  try {
    ...
    // Iterate over questions (not userAnswers) and look up each user answer
...
// @desc Reset a completed quiz so it can be retaken
...
export const retakeQuiz = async (req, res, next) => {
  try {
    ...
    await Quiz.findByIdAndUpdate(quiz._id, {
      $unset: { completedAt: 1 },
      $set: { score: 0, userAnswers: [] },
    });

    res.status(200).json({ success: true, message: "Quiz reset for retake", statusCode: 200 });
  } catch (error) {
    next(error);
  }
};
